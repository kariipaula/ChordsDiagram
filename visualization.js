...
function getData(callback) {
    d3.csv("trips.csv", function(error, cellData) {
      if (error) {
        console.error(error);
      }
    ...
    //   var zValues     = []; // to determine extent of values for scale
      var matrix      = []; 
      var labels      = [];
      ...
      d3.range(nUniqueNodes).forEach(function(row) {
        ...
        //   matrix[row][column] = {
        //     source:      currCell.source_country,
        //     sourceIndex: row,
        //     target:      currCell.target_country,
        //     targetIndex: column,
        //     z:           currCell.z,
        //   };
          matrix[row][column] = currCell.z;

        //   zValues.push(currCell.z);
        });
...
function makeVis(data) {
    ...
    var showTooltip = function(d) {
        console.log(data.labels[d.source.index], data.labels[d.target.index]);
        tooltip
            .style("opacity", 1)
            .html("Source: " + data.labels[d.source.index] + "<br>Target: " + data.labels[d.target.index])
